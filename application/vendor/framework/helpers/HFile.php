...
* 
* 例子：
* HFile::createDir('a/1/2/3');                    测试建立文件夹 建一个a/1/2/3文件夹
* HFile::createFile('b/1/2/3');                    测试建立文件        在b/1/2/文件夹下面建一个3文件
* HFile::createFile('b/1/2/3.exe');             测试建立文件        在b/1/2/文件夹下面建一个3.exe文件
* HFile::copyDir('b','d/e');                    测试复制文件夹 建立一个d/e文件夹，把b文件夹下的内容复制进去
* HFile::copyFile('b/1/2/3.exe','b/b/3.exe'); 测试复制文件        建立一个b/b文件夹，并把b/1/2文件夹中的3.exe文件复制进去
* HFile::moveDir('a/','b/c');                    测试移动文件夹 建立一个b/c文件夹,并把a文件夹下的内容移动进去，并删除a文件夹
* HFile::moveFile('b/1/2/3.exe','b/d/3.exe'); 测试移动文件        建立一个b/d文件夹，并把b/1/2中的3.exe移动进去                   
* HFile::unlinkFile('b/d/3.exe');             测试删除文件        删除b/d/3.exe文件
* HFile::unlinkDir('d');                      测试删除文件夹 删除d文件夹
*/
class HFile {
    ...
    */
    public static function createFile($aimUrl, $overWrite = false) {
           if (file_exists($aimUrl) && $overWrite == false) {
             ...
             HFile::unlinkFile($aimUrl);
           }
           $aimDir = dirname($aimUrl);
           HFile::createDir($aimDir);
           touch($aimUrl);
           return true;
    }
    /**
    ...
    public static function moveDir($oldDir, $aimDir, $overWrite = false) {
           ...
           while(false !== ($file = readdir($dirHandle))) {
             ...
             if (!is_dir($oldDir.$file)) {
                HFile::moveFile($oldDir . $file, $aimDir . $file, $overWrite);
             } else {
                HFile::moveDir($oldDir . $file, $aimDir . $file, $overWrite);
             }
           }
           closedir($dirHandle);
           return rmdir($oldDir);
    ...
    public static function unlinkDir($aimDir) {
           ...
           while(false !== ($file = readdir($dirHandle))) {
             ...
             if (!is_dir($aimDir.$file)) {
                HFile::unlinkFile($aimDir . $file);
             } else {
                HFile::unlinkDir($aimDir . $file);
             }
           }
           closedir($dirHandle);
           return rmdir($aimDir);
    ...
    */
    public static function copyDir($oldDir, $aimDir, $overWrite = false) {
           ...
           while(false !== ($file = readdir($dirHandle))) {
             ...
             if (!is_dir($oldDir . $file)) {
                HFile::copyFile($oldDir . $file, $aimDir . $file, $overWrite);
             } else {
                HFile::copyDir($oldDir . $file, $aimDir . $file, $overWrite);
             }
           }
